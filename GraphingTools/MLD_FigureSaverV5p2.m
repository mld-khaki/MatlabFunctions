function [ Status ] = MLD_FigureSaverV5p2( FigureHandle , SavePath , varargin)
...
%
%                   MLD_FigureSaverV5p2(FigureHandle,'.','filename',sprintf('Figure_%03u',ClsCtr),...
...
for eCtr=1:length(Extensions)
    if strcmpi(InputData.(Extensions{eCtr}),'true')==1
        Extension_List{eCtr} = [SavePath InputData.FileName '.' lower(Extensions{eCtr})];
    end
end
