function h2 = MLD_BreakPlot(x,y,y_break_start,y_break_end,break_type,y_arbitrary_scaling_factor)
...
myIndex =  y>y_break_start & y <y_break_end;
x(myIndex)=[];
y(myIndex)=[];

% MAP THE DATA
myIndex = y >= y_break_end;
y2 = y - myIndex.*y_break_width;
...
switch break_type
   case 'Patch', myIndex =  10.0;
   case 'RPatch',myIndex = 100.0;
   case 'Line',  myIndex =   2.0;
end
x_half_tick = diff(xlim(1:2))./myIndex;
switch break_type
   case {'Patch','RPatch'}
       xx = xlim(1) + x_half_tick.*(0:myIndex);
       switch break_type
           case 'Patch',yy = repmat( ...
                   [y_gap_mid+y_half_gap y_gap_mid-y_half_gap],1,floor(myIndex./2));
   ...
   case 'Line'
       ...
       for myIndex=0:2:2
           line(xx,y_gap_mid+([-1 2]+myIndex).*y_gap_width./2);
       end
end
set(gca,'xlim',xlim);
