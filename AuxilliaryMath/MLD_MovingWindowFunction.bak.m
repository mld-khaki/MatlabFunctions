...
for wCtr=Beg:Window-Overlap:End
	...
	if End2 < length(InpStream) && Beg2 > 0
		wsCtr = wsCtr + 1;
		WindowsStruct(wsCtr).Beg = Beg2;
		WindowsStruct(wsCtr).End = End2;
		WindowsStruct(wsCtr).Location = End2;
		WindowsStruct(wsCtr).Index = wsCtr;
	end
end
...
for jCtr=1:length(WindowsStruct)
	Beg = WindowsStruct(jCtr).Beg;
	End = WindowsStruct(jCtr).End;
	PartSig = InpStream(Beg:End);
	Out(jCtr) = Function(PartSig);
	OutInd(jCtr) = WindowsStruct(jCtr).Location;
end
